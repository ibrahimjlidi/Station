...
export function useMensuel(params?: { mois: number; annee: number }) { return useQuery({ queryKey: ['rapport-mensuel', params], queryFn: async () => (await api.get('/rapports/mensuel', { params })).data, enabled: Boolean(params?.mois && params.annee) }); }
export function useSoldesClients() { return useQuery<ClientBalance[]>({ queryKey: ['rapport-clients-soldes'], queryFn: async () => (await api.get<ClientBalance[]>('/rapports/clients-soldes')).data }); }
export function useSoldesFournisseurs() { return useQuery<SupplierBalance[]>({ queryKey: ['rapport-fournisseurs-soldes'], queryFn: async () => (await api.get<SupplierBalance[]>('/rapports/fournisseurs-soldes')).data }); }
export function useStockCuvesRapport() { return useQuery<StockReport[]>({ queryKey: ['rapport-stock-cuves'], queryFn: async () => (await api.get<StockReport[]>('/rapports/stock-cuves')).data }); }
